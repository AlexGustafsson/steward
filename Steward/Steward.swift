...
func index(roots: [URL], outputPath: URL, _ logCallback: @escaping @MainActor (LogEntry) -> Void)
...
func index(roots: [URL], _ logCallback: @escaping @MainActor (LogEntry) -> Void) throws -> Task<
  [IndexEntry], Error
> {
  let toolURL = Bundle.main.bundleURL
    .appendingPathComponent("Contents/MacOS/StewardTool")
...
func upload(
  root: URL, entries: [IndexEntry], _ logCallback: @escaping @MainActor (LogEntry) -> Void
) throws -> Task<Void, Error> {
  ...
  process.executableURL = toolURL
  process.arguments = [
    "--verbose", "upload", "--from", root.path(percentEncoded: false), "--to", credentials.bucket,
  ]
  process.environment = [
...
func diff(local: URL, remote: [IndexEntry], _ logCallback: @escaping @MainActor (LogEntry) -> Void)
  throws -> Task<[IndexEntry], Error>
{
  let toolURL = Bundle.main.bundleURL
    .appendingPathComponent("Contents/MacOS/StewardTool")
  ...
  process.executableURL = toolURL
  process.arguments = [
    "--verbose", "diff", "--output", "remote-only", local.path(percentEncoded: false), "/dev/stdin",
  ]
...
func download(
  ...
  process.executableURL = toolURL
  process.arguments = [
    "--verbose", "download", "--from", credentials.bucket, "--to", root.path(percentEncoded: false),
  ]
  process.environment = [
    "B2_REGION": credentials.region,
    "B2_KEY": credentials.key,
...
struct LogEntry: Codable, Identifiable {
  ...
  init(
    id: Int, time: Date, level: String, msg: String, error: String,
...
func readLogs(fileHandle: FileHandle, _ callback: @escaping @MainActor (LogEntry) -> Void) -> Task<
  Void, Error
> {
  return Task {
    let decoder = JSONDecoder()
